...
from ionoscloud_container_registry.api_response import ApiResponse, T as ApiResponseT
import ionoscloud_container_registry.models
from ionoscloud_container_registry import rest
from ionoscloud_container_registry.exceptions import (
    ApiValueError,
    ApiException,
    ApiFailedRequest,
    ApiTimeout,
...
class ApiClient:
    ...
    def __init__(
        ...
        self.user_agent = 'ionos-cloud-sdk-python/v2.0.0-beta.1'
            ...
            while True:
                ...
                try:
                    ...
                    if response_error.status in (
                        ...
                        backoff_time = self.configuration.wait_time
                    ...
                    if number_of_calls >= self.configuration.max_retries:
                        if self.configuration.debug:
                            logger.debug('number of maximum retries exceeded {}'.format(self.configuration.max_retries))
                        raise response_error

                    if backoff_time > self.configuration.max_wait_time:
                        backoff_time = self.configuration.max_wait_time
                    ...
                    time.sleep(self.configuration.wait_time)
    ...
    def __deserialize(self, data, klass):
        ...
        if isinstance(klass, str):
            if klass.lower().startswith('list['):
                m = re.match(r'(?:List|list)\[(.*)]', klass)
                assert m is not None, "Malformed List type definition"
                sub_kls = m.group(1)
                return [self.__deserialize(sub_data, sub_kls)
                        for sub_data in data]

            if klass.lower().startswith('dict['):
                m = re.match(r'(?:Dict|dict)\[([^,]*), (.*)]', klass)
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                klass = getattr(ionoscloud_container_registry.models, klass)
    ...
    @staticmethod
    def wait_for(fn_check, fn_request, timeout=3600, initial_wait=1, scaleup=10, max_wait=60, console_print=None):
        """
        ...
        while True:
            ...
            if wait_period < max_wait and current_time > next_increase:
                wait_period *= 2
                next_increase = time.time() + wait_period * scaleup
                scaleup *= 2
